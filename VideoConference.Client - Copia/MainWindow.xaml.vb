...
Class MainWindow
    ...
    Private _remoteCursorElement As System.Windows.Shapes.Path 'System.Windows.Shapes.Ellipse
    ...
    Public Sub New()
        ...
        AddHandler btnTestDirect.Click, AddressOf btnTestDirect_Click
    ...
    Private Async Sub btnDisconnect_Click(sender As Object, e As RoutedEventArgs)
        If _connection IsNot Nothing Then
            ...
            If _screenShareManager IsNot Nothing Then
                ...
                _screenShareManager = Nothing  ' IMPORTANTE
            End If
    ...
    Private Sub UpdateRemoteCursorPosition(x As Integer, y As Integer)
        Try
            ...
            ' Calcola l'area effettiva dell'immagine (potrebbe avere letterbox/pillarbox)
            Dim renderRect As New Rect()
            Dim scale As Double

            If imageAspect > screenAspect Then
                ' L'immagine è più larga - ci sono bande sopra/sotto
                scale = imageWidth / screenWidth
                renderRect.Width = imageWidth
                renderRect.Height = screenHeight * scale
                renderRect.X = 0
                renderRect.Y = (imageHeight - renderRect.Height) / 2
                Debug.Print($"📐 Caso 1 (letterbox verticale): scale={scale:F3}, rect Y={renderRect.Y:F2}")

            Else
                ' L'immagine è più alta - ci sono bande ai lati
                scale = imageHeight / screenHeight
                renderRect.Height = imageHeight
                renderRect.Width = screenWidth * scale
                renderRect.X = (imageWidth - renderRect.Width) / 2
                renderRect.Y = 0
                Debug.Print($"📐 Caso 2 (pillarbox orizzontale): scale={scale:F3}, rect X={renderRect.X:F2}")

            End If

            ' Converti le coordinate del mouse del mittente in coordinate dell'immagine
            ' usando la scala calcolata
            Dim cursorX = renderRect.X + (x * scale)
            Dim cursorY = renderRect.Y + (y * scale)

            Debug.Print($"📍 Posizione calcolata: ({cursorX:F2}, {cursorY:F2})")

            ' Posiziona il cursore (centrato)
            Dim cursorSize = _remoteCursorElement.Width / 2
            Canvas.SetLeft(_remoteCursorElement, cursorX - cursorSize)
            Canvas.SetTop(_remoteCursorElement, cursorY - cursorSize)
